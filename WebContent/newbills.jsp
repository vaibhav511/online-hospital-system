...
<script type="text/javascript">
	function checkDate() {
		...
		if (myDate > today) {
			...
			alert("Date should be greater then today and should be in format(Chrome : dd-mm-yyyy || IE: yyyy-mm-dd)."
					...
					<nav class="navbar navbar-default">
						...
						<div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
							...
							<%}if((acctype!=null) && (acctype.equals("Admin"))){ %>
								...
								<li><a href="bills.jsp" class="active">Bills</a></li>
		...
		<div class="container">
			<div class="login-body">
				<div class="login-heading">
					<h1>New Bill Generation</h1>
				</div>
				<div class="login-info">
					<form action="BillRegistration" method="post">
					<%if(request.getAttribute("success")!=null){%>
					<label name="success" style="color: red; font-family: sans-serif;"><%=request.getAttribute("success")%></label>
					<%} %>
					
					<label  class="user">Select Patient</label> <select  class="user" id="patientname"
							name="txtPatientName" required>
						...
						<label  class="user">Select Date of Admit</label> 
						<input  class="user" type="date" required name="txtAdmitdate" name="appdate">
						<label>Select Date of Discharge</label>
						<input type="date"  onchange="checkDate()" required name="txtDischargedate" name="appdate">
						<input type="text" class="user" pattern="\d*" title='Room charges should be integer'  name="txtRoomCharges" placeholder="Room Charges" required="">
						<input type="text" class="user" pattern="\d*" title='Pathalogy fee should be integer'  name="txtPathalogyFee" placeholder="Pathlogy Fee">
						<input type="text" class="user" pattern="\d*" title='Doctor fee should be integer' name="txtDoctorFee" placeholder="Doctor Fee" required="">
						<input type="text" class="user" pattern="\d*" title='Total ammount should be integer'  name="txtTotalAmt" placeholder="Total Amount" required="">
						<input type="text" class="user" name="txtRemarks" placeholder="Remarks">
